<a class="btn btn-danger" id="bulk_delete_btn"><i class="facilitador-trash"></i> <span>{{ __('pedreiro::generic.bulk_delete') }}</span></a>
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">
                    <i class="facilitador-trash"></i> {{ __('pedreiro::generic.are_you_sure_delete') }} <span id="bulk_delete_count"></span> <span id="bulk_delete_display_name"></span>?
                </h4>
            </div>
            <div class="modal-body" id="bulk_delete_modal_body">
            </div>
            <div class="modal-footer">
                <form action="{{ route('rica.facilitador.'.$dataType->slug.'.index') }}/0" id="bulk_delete_form" method="POST">
                    {{ method_field("DELETE") }}
                    {{ csrf_field() }}
                    <input type="hidden" name="ids" id="bulk_delete_input" value="">
                    <input type="submit" class="btn btn-danger float-right delete-confirm"
                             value="{{ __('pedreiro::generic.bulk_delete_confirm') }} {{ strtolower($dataType->getTranslatedAttribute('display_name_plural')) }}">
                </form>
                <button type="button" class="btn btn-secondary float-right" data-dismiss="modal">
                    {{ __('pedreiro::generic.cancel') }}
                </button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
...
window.onload = function () {
    ...
    $bulkDeleteBtn.click(function () {
        ...
        if (count) {
            ...
            // No row selected
            toastr.warning('{{ __('pedreiro::generic.bulk_delete_nothing') }}');
        }
    });
}
</script>
