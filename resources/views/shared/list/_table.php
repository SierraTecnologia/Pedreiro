...
if ($listing->count()) {
    ...
    && (app('facilitador.user')->can('destroy', $controller)
...
<table class="table listing columns-<?php echo count($columns)?>">
    <thead>
            <tr>
                ...
                <?php foreach(array_keys($columns) as $column): ?>
                    <th class="<?php echo strtolower($column)?>"><?php echo $column?></th>
                <?php endforeach ?>

                <?php if (isset($test_actions)) : ?>
                    <?php if (count($test_actions)) : ?>
                        <th class="actions-<?php echo count($test_actions)?>"><?php echo __('pedreiro::list.table.actions') ?></th>
                    <?php endif ?>
                <?php else: ?>
                    <th class="actions-3"><?php echo __('pedreiro::list.table.actions') ?></th>
                <?php endif ?>
    ...
    <tbody>
        ...
        <?php if ($can_delete && $many_to_many) : ?>
            ...
            <?php echo __('pedreiro::list.table.remove_selected') ?>
                    </a>
                </td>
            </tr>
        ...
        foreach ($listing as $item): ?>
            ...
            <tr data-model-id="<?php echo $item->getKey()?>" class="<?php echo $item->getAdminRowClassAttribute()?>"
            <?php
            // Render parent id
    ...
    && $item->trashed()
            ...
            <?php foreach(array_values($columns) as $i => $column): ?>
                    <td class="<?php echo strtolower($column_names[$i])?>">
            ...
            <?php if (count($test_actions)) : ?>
                    <td class="actions">
                <?php echo implode(' | ', $item->makeAdminActions($__data))?>
                    </td>
            <?php endif ?>
