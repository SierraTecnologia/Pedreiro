    ...
    @if(class_exists($options->model) || $options->type == 'morphTo')
        ...
        @if($options->type == 'morphTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @if(isset($query))
                    <p>
                        <a href="{{\Pedreiro\Routing\UrlGenerator::managerRoute($relationshipData->{$options->model}, 'show', $relationshipData->{$options->column})}}">
                        {{ $query->{$model->getApresentationNameKey()} }}
                        </a>
                    </p>
                @else
                    <p>{{ __('pedreiro::generic.no_results') }}</p>
                @endif
            ...
            @else
                <select
                    ...
                    @if(isset($options->taggable) && $options->taggable === 'on')
                        ...
                        data-error-message="{{__('bread.error_tagging')}}"
                    @endif
                >
