@if(isset($options->relationship))
    ...
    @if( !method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        <p class="label label-warning"><i class="facilitador-warning"></i> {{ __('pedreiro::form.field_select_dd_relationship', ['method' => \Illuminate\Support\Str::camel($row->field).'()', 'class' => $dataType->model_name]) }}</p>
    @endif
    ...
    @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            @if(isset($options->options))
                <optgroup label="{{ __('pedreiro::generic.custom') }}">
            ...
            <optgroup label="{{ __('pedreiro::database.relationship.relationship') }}">
